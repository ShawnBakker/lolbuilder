...
Amended 2026-07-03 (PC-5 finding, see `policy-conflict-amendment.md`): Riot's General Policies explicitly prohibit scraping "data from undocumented endpoints or any other sources outside of the provided Riot API Endpoints and other documented Third Party Developer Tools" — D2/D3's sourcing operates against this stated clause, not merely lolalytics' own notice. This is the strongest reason D1 is permanent: enforcement risk is assessed low only at zero-visibility personal scale.
Evidence: lolalytics API notice (a3 subdomain) [external constraint]; Riot General Policies "Please Don't" clause [external constraint, read directly during PC-5 registration].
...
(b) Champion/build/counters: `lolalytics.com/lol/<name>/build/q-data.json` (and `/counters/q-data.json`, incl. `?vslane=` cross-lane variants, empirically confirmed) — Qwik route-loader payloads containing baseline WR/PR/BR/n, per-rank daily series, item sets with wr/n, runes, skills, and `time`/`timeWin` 7-bucket game-length win data (objects keyed "1"–"7", not arrays); container slots (object fields, array elements) hold only base-36 refs — primitives live as top-level `_objs` entries — so consumers resolve every leaf, single hop *(corrected 2026-07-03: an earlier "mixed literal/ref" report was an inference, refuted at fixture scale)*; served to datacenter IPs even though the HTML site is bot-walled.
...
Reverses if: endpoints churn — fallback chain is u.gg, then op.gg MCP; sustained breakage >1 incident/month triggers downscoping the analytics half entirely (pre-committed criterion). Fallback #1 documented via a maintained open-source client rather than a captured endpoint (PC-3, 2026-07-03): `kade-robertson/uggo` (MIT, crates.io `uggo-ugg-api` v0.6.1+, ~12 releases since Nov 2023) — if reversal happens, the live contract is readable from the library's published source, no re-capture needed. Stronger reliability profile than fallback #2 (op.gg MCP: tool-interface only, synergy coverage unconfirmed).
...
Upgraded post-review [project empirical, corrected 2026-07-03]: all container slots are base-36 refs (primitives live only as top-level `_objs` entries; verified at fixture scale after an earlier "mixed literal/ref" inference was refuted) — resolution must be leaf-level and single-hop, invariants run post-resolution, and any non-string container slot is itself a format-violation alarm (spec AC-8/9/10).
...
4. *(Reworded 2026-07-03 — see `policy-conflict-amendment.md`.)* The data sourcing **operates against an explicit Riot policy clause** (the General Policies' prohibition on scraping undocumented/non-Riot endpoints, agreed to at PC-5 key registration) — not a gray area. Risk mitigation is scale and visibility (private, unlisted, personal-only, polite cadence), not compliance. Accepted with eyes open; it is the reason D1 is permanent. The policy's own App Note channel exists as an optional path to an actual answer.
