...
2. **Fixtures:** commit ≥2 raw captured payloads (fresh polite refetches, 4 requests, within posture) and verify the leaf-encoding model against them — all container slots ref-encoded. *(Amended 2026-07-03: the original dual-interning-mode coverage requirement was voided when fixture-scale scanning refuted the mixed-interning claim; the uniform-ref invariant replaced it.)*
