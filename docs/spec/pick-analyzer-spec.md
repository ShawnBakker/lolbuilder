...
- AC-8: Resolves the Qwik object graph mechanically (flat `_objs`, base-36 refs) for exactly the tables we consume — not a general Qwik deserializer. **Resolution must occur at leaf level**: interning is mixed and non-deterministic within a single payload — identical row shapes appear with literal numbers in some rows and base-36 refs in others (observed 2026-07-02; caused a NaN in the reviewer's first computation).
- AC-9: Named invariants fail loudly with the violated assumption in the message, and **run post-resolution** (pre-resolution type checks false-alarm on ref-valued leaves): root `_entry` resolves; `time`/`timeWin` present as objects keyed "1"–"7" whose values resolve to numeric games/wins with wins ≤ games; `team_h` columns ⊇ {id, wr, d1, d2, pr, n}; matchup tables non-empty for a known-popular champion.
- AC-10: Golden-fixture tests: ≥2 captured raw payloads committed, **chosen to cover both interning modes** (the 2026-07-02 review's saved probe payloads are candidates); CI runs deserializer against fixtures; a scheduled weekly CI job refetches one live payload and diffs shape — running **from GitHub Actions** so it doubles as the D5 same-IP-class canary.
