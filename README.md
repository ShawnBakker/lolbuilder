...
## Champ-select auto-fill (v2, M7)

A small local helper (`apps/helper`) lets the site auto-populate the draft
board from a live champ select — read-only, local-only, plain readable
JavaScript. Friends: see [`apps/helper/INSTALL.md`](apps/helper/INSTALL.md).
Without it, the site is fully usable with manual entry.
